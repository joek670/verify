...
[`docs/dsh-openrouter-windows.md`](docs/dsh-openrouter-windows.md) is parked for the same
reason, though it sits under `docs/` rather than here: it records how to hand DSH an
OpenRouter API key on Windows through the environment that launched it. Nothing in this
project reads that key or talks to OpenRouter.
